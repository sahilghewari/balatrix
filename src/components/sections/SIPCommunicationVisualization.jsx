...
const DESKTOP_STEP_DEFINITIONS = [
  ...
  { id: 'bye', label: 'BYE', subLabel: '1800-XXX-XXXX', color: COLORS.red, path: 'M 650 226 C 505 176, 288 345, 112 259', source: 'callee', target: 'caller', weight: 560 },
  { id: 'final-ok', label: '200 OK', subLabel: '1800-XXX-XXXX', color: COLORS.green, path: 'M 112 253 C 292 300, 510 188, 650 224', source: 'caller', target: 'callee', weight: 550 },
];
...
const MOBILE_STEP_DEFINITIONS = [
  ...
  { id: 'bye', label: 'BYE', subLabel: '1800-XXX-XXXX', color: COLORS.red, path: 'M 320 660 C 20 600, 20 200, 80 125', source: 'callee', target: 'caller', weight: 750 },
  { id: 'final-ok', label: '200 OK', subLabel: '1800-XXX-XXXX', color: COLORS.green, path: 'M 80 125 C 380 200, 380 600, 320 660', source: 'caller', target: 'callee', weight: 750 },
];
...
const SIPCommunicationVisualization = ({ background = false, scrollTarget = null }) => {
  ...
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      if (background) {
        ...
        tl.to(containerRef.current, { opacity: 1, duration: 0.1, ease: 'none' })
          .to(containerRef.current, { opacity: 1, duration: 0.8, ease: 'none' })
          .to(containerRef.current, { opacity: 0, duration: 0.1, ease: 'none' });
      } else {
        ScrollTrigger.create({
          trigger: scrollTarget?.current || containerRef.current,
          start: scrollTarget ? 'top top' : 'top 82%',
          end: scrollTarget ? 'bottom bottom' : 'bottom 18%',
          scrub: 0.55,
  ...
  const fadeOut = sceneProgress < 0.04
    ? clamp(sceneProgress / 0.04)
    : (sceneProgress > 0.96 ? clamp((1 - sceneProgress) / 0.04) : 1);
  ...
  /* Elapsed seconds for node float animation */
  const floatTime = performance.now() / 1000;

  /* Pre-compute float offsets for all nodes — shared by node rendering AND path offsetting */
  const nodeFloatOffsets = {};
  if (background) {
    for (const id of Object.keys(activeNodes)) {
      const fp = NODE_FLOAT_PARAMS[id];
      if (fp) {
        nodeFloatOffsets[id] = {
          x: fp.ax * Math.sin(fp.fx * floatTime + fp.px) + fp.ax * 0.35 * Math.cos(fp.fx * 1.6 * floatTime + fp.py),
          y: fp.ay * Math.cos(fp.fy * floatTime + fp.py) + fp.ay * 0.35 * Math.sin(fp.fy * 1.4 * floatTime + fp.px + 1.0),
        };
      }
    }
  }
  ...
  return (
    <div ref={containerRef} className={background ? 'relative h-full min-h-full w-full overflow-hidden bg-transparent' : 'relative min-h-[420px] w-full overflow-hidden rounded-[18px] border border-white/10 bg-[#050914] shadow-[0_32px_90px_rgba(0,0,0,0.48)]'}>
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_18%,rgba(59,130,246,0.13),transparent_28%),radial-gradient(circle_at_78%_70%,rgba(34,197,94,0.10),transparent_30%)]" />
      ...
      >
        ...
        <rect width={isMobile ? 400 : 760} height={isMobile ? 800 : 470} fill="url(#network-dots)" opacity={0.8 * fadeOut} />
        <g opacity={0.65 * fadeOut} fill="#7dd3fc">
          {(isMobile
        ...
        <g opacity={fadeOut} fill="none" strokeLinecap="round">
        ...
        {Object.entries(activeNodes).map(([id, node]) => {
          ...
          return <g key={id} opacity={fadeOut} transform={`translate(${offset.x}, ${offset.y})`}>
